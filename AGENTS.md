...
- Choose the matching template in `.github/PULL_REQUEST_TEMPLATE/` when opening a PR:
  - **code.md** — default for behavior, schema, Move, or multi-file changes
  - **docs.md** — docs-only or agent-config tweaks
- **Summary** — required: what changed and why.
- **Related work** — link stacked PRs, issues, or follow-ups when applicable; delete the section if none.
- **Scope** — one line naming the primary surface(s) touched (code template only).
- **Verification** — required: commands run and results, or `Not run: <reason>`.
- **Reviewer notes** — optional block for manual testing, trust/release impact, or screenshots; delete the section if none apply.
- **Do not** add tool attribution lines such as "Made with Cursor" or "Generated by Codex".
- See `CONTRIBUTING.md` → Submitting a pull request for more detailed guidance.
