...
import { listDappIndexFixtures } from '../src/content/dappIndexFixtures';

describe('filterDappIndexEntries', () => {
  test('filters by category and search tokens', () => {
    const entries = listDappIndexFixtures();
    ...
    expect(
      filterDappIndexEntries(entries, 'library metadata', undefined, undefined).map(
        (entry) => entry.id,
      ),
